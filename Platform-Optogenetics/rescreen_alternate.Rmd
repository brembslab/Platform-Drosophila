...
for(x in 1:length(groupedPIs))
{
  ...
  groupedPIs[[x]]$ave_tr <- rowMeans(subset(groupedPIs[[x]], select = trainingvariables)) #2 average all training values
  groupedPIs[[x]]$te_pt <- groupedPIs[[x]]$test4-groupedPIs[[x]]$pretest #3 subtract pretest from final test ("test" in rescreendsign)
  groupedPIs[[x]]$ltr_pt <- groupedPIs[[x]]$training5-groupedPIs[[x]]$pretest #4 subtract pretest from last training ("training8" in rescreendesign)
...
PIs$last_training<-unlist(rbind(lapply(groupedPIs, "[", , lasttraining)))
PIs$test<-unlist(rbind(lapply(groupedPIs, "[", , "test4"))) #"test" in rescreendesign
...
## Tests against zero: last training
...
## Tests against zero: average training periods

```{r Wilcoxon_average, echo=FALSE, error=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.height = 7, fig.width = 16, comment=NA, results='asis'}

  #do wilcoxon tests
  wilcoxon<-numeric()
  groupnames=sort(names(groupedPIs))
  for(x in 1:length(groupedPIs)){wilcoxon[x] = signif(wilcox.test(PIs$average_training[PIs$group==groupnames[x]])$p.value, 3)} #test all groups against zero
  
  #compute Bayes Factors
          results.bayes<-list()
          for(x in 1:length(groupedPIs)){results.bayes[[x]]=extractBF(ttestBF(PIs$average_training[PIs$group==groupnames[x]]))} #extract BayesFactors for all groups
          results.bayes<-do.call("rbind", results.bayes) #fuse all Bayes results into one dataframe
          results.bayes <- results.bayes[-c(3,4)]# drop the date and code columns
          row.names(results.bayes) <- groupnames #group name as row name
          results.bayes <- signif(results.bayes, digits=3) # reduce results to 3 significant digits
...
# plot PI box plot test against zero
  plots.singles<-list(ggplot(PIs, aes(group, average_training)) +
    geom_hline(yintercept = 0, colour = "#887000", size = 1.2) +
    geom_boxplot(fill = boxcolors, notch = FALSE, outlier.color=NA, width=0.9, size=0.7) +
    geom_jitter(data=PIs, aes(group, last_training), position=position_jitter(0.3), shape=21, size=3, alpha=0.5, fill="grey") +
    scale_y_continuous(breaks = seq(-1, 1, .2)) +
    theme_light(base_size = 16) + theme(panel.grid.minor = element_blank(), panel.grid.major.x = element_blank() ,panel.border = element_rect(size = 0.5, linetype = "solid", colour = "black", fill=NA)) +
    theme(axis.text.y = element_text(size=18), axis.text.x = element_text(size=18, angle = 45, hjust = 1))+ ylab("PI of averaged training periods [rel. units]")+ xlab("Groups")+ theme(aspect.ratio=0.5)+
    samplesizes.annotate(boxes, samplesizes) +
    wilcox.annotate(boxes, wilcoxon))
